const calibrationDocument = `gtlbhbjgkrb5sixfivefivetwosix
...
const calibrationDocumentArray = calibrationDocument.split("\n")

const isNumberr= (character: string) => {
...
let sum = 0;
calibrationDocumentArray.map(text => {
    const digitsArray = [];
    
 for ( let i=0; i<= text.length ; i++) {
    if(isNumberr(text[i])) {
        digitsArray.push([Number(text[i]),i])
    }
 }
 ...
 for (const key in numbers) {
  ...
  if (index > -1) {
      digitsArray.push([numbers[key], index])
  }
 }

digitsArray.sort((a, b) => a[1] - b[1]);

const calibrationValue = digitsArray[0][0] * 10 + digitsArray[digitsArray.length-1][0]
sum = sum + calibrationValue
})

console.log("sssummmmmm", sum)